...
- 最近比赛列表与单场比赛详情。
- 最近 20 场会后台补齐十人阵容、赛后指标、最终装备、技能加点顺序和可用的真实购买事件；配置 STRATZ 后可进一步补充带游戏时间的加点与购买记录。
- 单场详情提供概览、发育、战斗、目标与构筑五页签；十人玩家选择状态在页签间共享，切换不触发新的页面请求。
- 已解析比赛可展示玩家金币/经验/补刀/反补时间轴、双方团队发育总量、天辉经济/经验优势、击杀日志、伤害对象与来源、目标事件和团战摘要；每个区块独立标记 complete、partial 或 unavailable。
...
- 完整 replay 下载与自研解析。
- 眼位、死亡和移动路径热图。
- 完整死亡/助攻账本、死亡金钱损失、逐次治疗、伤害类型、Buff/Debuff 持续时间与完整物品出售账本。
- 带可解释样本范围的同英雄基准、胜率预测和版本化 MVP 模型。
...
```text
OpenDota public matches / STRATZ replay-derived enrichment
Dota 2 official current-data / curated maps
        |
        v
...
Fastify REST API
        ...
        |
        +---- Supabase PostgreSQL (compact matches + parsed analysis sidecar)
        |
        v
...
- [Match analysis sidecar ADR](docs/adr/0007-match-analysis-sidecar.md)
...
packages/dota-data   OpenDota、STRATZ 与 Dota 2 official current-data 适配、账号解析和质量标准化
...
| `STRATZ_API_BASE_URL` | STRATZ GraphQL | 可选的服务端比赛增强 endpoint |
| `STRATZ_TOKEN` | empty | 可选的服务端 STRATZ Token；未配置时完全禁用增强 |
...
- 只使用上游允许公开访问的数据。
- 不绕过隐藏比赛历史或私密资料。
- OpenDota Key、STRATZ Token 和数据库凭据只存在于服务端环境。
