...
- Deploy `apps/api` as one always-running Railway service in Singapore for the MVP preview.
- Retain the Fly Machine configuration for a later Tokyo migration when Fly API access is available.
...
- Railway must keep one service replica running and must not sleep the only instance.
...
4. Railway runs one always-on API replica in Singapore; the Fly configuration remains ready for a later `nrt` migration.
